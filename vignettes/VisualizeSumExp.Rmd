...
In this vignette, we will build a shiny app to visualize `RangeSummarizedExperiment` using epivizrChart. Since epivizrChart package uses the web components framework, it can be integrated with most frameworks that support HTML. 
...
We create an Environment element which visualizes genome wide data. We then visualize `cancer` and `normal` valuues from the summarized experiment object.
...
After looking at the genomic wide data, if you are interested in further exploring a specific region of the genome, We can create a navigation element linked to that genomic location. We can plot additional annotation/data charts to understand the data in this region. 
