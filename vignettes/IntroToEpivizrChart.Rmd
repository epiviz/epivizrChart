...
epivizEnv_2 <- epivizEnv(chr="chr11")

# add a line track from tcga_colon_curves object to the environment
means_track <- epivizEnv_2$plot(tcga_colon_curves, datasource_name="450kMeth", type="bp", columns=c("cancerMean","normalMean"))

# add a scatter plot from the summarized experiment object
...
blocks_track2 <- epivizNav$plot(tcga_colon_blocks, datasource_name="450kMeth")

epivizEnv_2
```

If we want a navigation element to include all of the environment's charts at a particular genomic region, we can use the environment's `init_region`.
```{r}
epivizNav <- epivizEnv_2$init_region(chr="chr11", start=99800000, end=103383180)
